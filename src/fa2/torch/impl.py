...
def fa2_forward_torch(q, k, v, causal, softmax_scale, br, bc):
    bh, n, d = q.shape
    ...
    for bh_idx in range(bh):
        for row_start in range(0, n, br):
            ...
            m_i = torch.full((row_end - row_start,), neg_inf, device=q.device)
            ...
            for col_start in range(0, n, bc):
                ...
                o_block = (torch.exp(m_i - m_new) * l_i / l_new)[:, None] * o_block + torch.matmul(p, v_block) / l_new[:, None]
...
def fa2_backward_torch(q, k, v, o, do, lse, causal, softmax_scale, br, bc):
    bh, n, d = q.shape
    ...
    for bh_idx in range(bh):
        for row_start in range(0, n, bc):
            ...
            for col_start in range(0, n, br):
                ...
                oi = o[bh_idx, col_start:col_end].to(torch.float32)
                ...
                dq_block = torch.matmul(dp, kj) * softmax_scale
                dk_block = torch.matmul(dp.transpose(-2, -1), qi) * softmax_scale
                dv_block = torch.matmul(p.transpose(-2, -1), dp)

                dq[bh_idx, col_start:col_end] = dq[bh_idx, col_start:col_end] + ds @ kj
                dk_j = ds.transpose(0, 1) @ qi
...
def fa2_torch(q, k, v, causal, softmax_scale, spec):
    qb = _merge_bh(q)
    kb = _merge_bh(k)
    vb = _merge_bh(v)
