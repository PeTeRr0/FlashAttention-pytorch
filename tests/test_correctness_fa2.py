...
from fa2.cuda.impl import fa2_cuda
from fa2.spec import pick_fa2_spec
from fa2.torch.impl import fa2_backward_torch, fa2_forward_torch, fa2_torch
from tests.utils import dtype_tolerances, flatten_lse, flatten_output, make_qkv


@pytest.mark.parametrize("shape", [(1, 1, 24, 32), (2, 2, 33, 64)])
@pytest.mark.parametrize("causal", [False, True])
@pytest.mark.parametrize("merge_heads", [False, True])
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
def test_fa2_torch_forward_matches_reference(shape, causal, merge_heads, dtype, device):
    ...
    o, lse = fa2_torch(q, k, v, causal, softmax_scale, spec)
...
@pytest.mark.parametrize("causal", [False, True])
def test_fa2_torch_backward_matches_reference(causal, device):
    torch.manual_seed(11)
    dtype = torch.float32
    batch, heads, seqlen, head_dim = 1, 2, 16, 40
    q, k, v = make_qkv(batch, heads, seqlen, head_dim, device=device, dtype=dtype, merge_heads=True)
    softmax_scale = head_dim ** -0.5
    spec = pick_fa2_spec(head_dim)

    o, lse = fa2_forward_torch(q, k, v, causal, softmax_scale, spec.br, spec.bc)
    do = torch.randn_like(o)

    dq, dk, dv = fa2_backward_torch(q, k, v, o, do, lse, causal, softmax_scale, spec.br, spec.bc)
...
@pytest.mark.parametrize("causal", [False, True])
def test_fa2_triton_forward_matches_reference(causal, triton_available):
    ...
    from fa2.triton.impl import fa2_triton

    spec = pick_fa2_spec(head_dim)
    o, lse = fa2_triton(q, k, v, causal, softmax_scale, spec)
    o_ref, lse_ref = reference_attention(q, k, v, causal=causal, softmax_scale=softmax_scale)
...
@pytest.mark.parametrize("causal", [False, True])
def test_fa2_cuda_backward_matches_reference(causal, cuda_extension_available):
    if not cuda_extension_available:
        pytest.skip("CUDA extension for FlashAttention-2 not built or no GPU available")

    torch.manual_seed(13)
    dtype = torch.float16
    device = "cuda"
    batch, heads, seqlen, head_dim = 1, 2, 32, 48
    q, k, v = make_qkv(batch, heads, seqlen, head_dim, device=device, dtype=dtype, merge_heads=True)
    ...
    softmax_scale = head_dim ** -0.5
    spec = pick_fa2_spec(head_dim)
    o, _ = fa2_cuda(q, k, v, causal, softmax_scale, spec)
    do = torch.randn_like(flatten_output(o))

    loss = (flatten_output(o) * do).sum()
    loss.backward()
...
def test_fa2_backend_consistency(triton_available, cuda_extension_available):
    ...
    if len(backends) < 2:
        pytest.skip("Only one FlashAttention-2 backend available on this machine")

    torch.manual_seed(14)
    batch, heads, seqlen, head_dim = 1, 2, 28, 32
    ...
    for backend in backends:
        spec = pick_fa2_spec(head_dim)
        if backend == "torch":
            o, lse = fa2_torch(q, k, v, True, softmax_scale, spec)
        elif backend == "triton":
            from fa2.triton.impl import fa2_triton

            o, lse = fa2_triton(q, k, v, True, softmax_scale, spec)
        else:
            o, lse = fa2_cuda(q, k, v, True, softmax_scale, spec)
