...
def method_name(backend: str) -> str:
    ...
    if backend == "triton":
        return "FlashAttention-1 Triton"
    if backend == "cuda":
        return "FlashAttention-1"
    return f"fa1-{backend}"


def main():
    parser = argparse.ArgumentParser(description="Benchmark FlashAttention-1 backends")
    add_common_args(parser)
    parser.add_argument("--tag", type=str, default="fa1", help="Base name for result files")
    ...
    if not backends:
        raise SystemExit("No available backends for FlashAttention-1 on this machine")
